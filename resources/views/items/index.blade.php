    ...
    <div class="container mx-auto pt-24 px-4">
        ...
        --}}
            <form action="{{ route('items.index') }}" method="GET" class="mt-8 max-w-2xl mx-auto">
                ...
                --}}
                    <button type="submit"
                        class="absolute right-2 top-1/2 -translate-y-1/2 inline-flex items-center rounded-full bg-red-800 px-6 py-2 text-sm font-semibold text-white hover:bg-red-700">
                        Cari
                    </button>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                ...
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        @forelse ($foundItems as $item)
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-6 text-center">
                                    {{ \Carbon\Carbon::parse($item->tanggal_penemuan)->format('d M Y') }}</td>
                                {{-- Kolom Status Badge --}}
                                {{-- GANTI BAGIAN INI --}}
                                <td class="py-3 px-6 text-center">
                                    @if ($item->status == 'Belum Diambil')
                                        {{-- Tambahkan whitespace-nowrap dan inline-block --}}
                                        <span
                                            class="bg-yellow-200 text-yellow-700 py-1 px-3 rounded-full text-xs font-semibold whitespace-nowrap inline-block">
                                            {{ $item->status }}
                                        </span>
                                    @else
                                        <span
                                            class="bg-green-200 text-green-700 py-1 px-3 rounded-full text-xs font-semibold whitespace-nowrap inline-block">
                                            {{ $item->status }}
                                        </span>
                                    @endif
                                </td>
                                    ...
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            @if (auth()->user()->isAdmin())
                                                ...
                                                <a href="{{ route('admin.reports.found.show', ['foundItem' => $item->uuid]) }}"
                                                ...
                                                {{-- Tombol Edit --}}
                                                <a href="{{ route('admin.reports.found.edit', ['foundItem' => $item->uuid]) }}"
                                                ...
                                                {{-- Form Delete --}}
                                                <form
                                                    action="{{ route('admin.reports.found.destroy', ['foundItem' => $item->uuid]) }}"
                                                    method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                        class="delete-button w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer"
                                                        title="Hapus">
                                            ...
                                            @else
                                                ...
                                                <a href="{{ route('items.show.found', ['foundItem' => $item->uuid]) }}"
                                                    class="w-5 mr-2 transform hover:text-blue-500 hover:scale-110"
                                                    title="Detail">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                        stroke-width="2" stroke="currentColor">
                        ...
                        @empty
                            {{-- Row Kosong --}}
                            <td colspan="{{ auth()->check() ? '6' : '5' }}" class="text-center py-4">Belum ada barang
                                temuan yang dilaporkan.</td>
                        @endforelse
                    </tbody>
                </table>
            </div>
            {{-- Pagination --}}
            <div class="mt-6">{{ $foundItems->appends(request()->query())->links('vendor.pagination.tailwind-white') }}
            </div>
        </div>
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        @forelse ($lostItems as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-6 text-center">
                                    {{ \Carbon\Carbon::parse($item->tanggal_kehilangan)->format('d M Y') }}</td>
                                {{-- GANTI BAGIAN INI --}}
                                <td class="py-3 px-6 text-center">
                                    @if ($item->status == 'Masih Hilang')
                                        {{-- Tambahkan whitespace-nowrap dan inline-block --}}
                                        <span
                                            class="bg-red-200 text-red-700 py-1 px-3 rounded-full text-xs font-semibold whitespace-nowrap inline-block">
                                            {{ $item->status }}
                                        </span>
                                    @else
                                        <span
                                ...
                                @auth
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            @if (auth()->user()->isAdmin())
                                                {{-- AKSI ADMIN --}}
                                                <a href="{{ route('admin.reports.lost.show', ['lostItem' => $item->uuid]) }}"
                                                    class="w-5 mr-2 transform hover:text-blue-500 hover:scale-110"
                                                    title="Detail">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                ...
                                                <a href="{{ route('admin.reports.lost.edit', ['lostItem' => $item->uuid]) }}"
                                                    class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110"
                                                    title="Edit">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <form
                                                    action="{{ route('admin.reports.lost.destroy', ['lostItem' => $item->uuid]) }}"
                                                    method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                            ...
                                            @else
                                                {{-- AKSI USER --}}
                                                <a href="{{ route('items.show.lost', ['lostItem' => $item->uuid]) }}"
                                                    class="w-5 mr-2 transform hover:text-blue-500 hover:scale-110"
                                                    title="Detail">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        ...
                        @empty
                            <td colspan="{{ auth()->check() ? '6' : '5' }}" class="text-center py-4">Belum ada barang
                                hilang yang dilaporkan.</td>
                        @endforelse
                    </tbody>
                </table>
            </div>
            {{-- Pagination --}}
            <div class="mt-6">{{ $lostItems->appends(request()->query())->links('vendor.pagination.tailwind-white') }}
            </div>
        </div>
    </div>
@endsection
